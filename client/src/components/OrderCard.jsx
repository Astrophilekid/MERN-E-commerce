...
const OrderCard = ({ order, openModal }) => {
  ...
  return (
    <div
      className={`flex max-h-fit max-sm:pr-4 cursor-pointer h-32  mb-3 rounded-lg border-violet-100 p-2 border  transition-all justify-between w-full lg:w-[90%] xl:w-[80%]`}
    >
      {/* left */}
      {confirmModal && (
      ...
      {products.length > 0 && (
        <div
          className="w-1/6 flex flex-col bg-white justify-center rounded-md h-full p-2"
          onClick={() => {
            openModal(order)
          }}
        >
          <img
            src={products[0].product.images[0]}
            alt="cart product image"
            className="object-contain max-h-full"
          />
          {products.length > 1 && (
            <h1 className=" ml-1 flex whitespace-nowrap text-sm text-gray-500">
